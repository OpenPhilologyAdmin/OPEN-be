...
RSpec.describe TokensManager::Resizer::Preparers::ValueBeforeAndAfterTokenCalculator, type: :model do
  let(:service) { described_class.new(selected_tokens:, token:, value_before_selected_text:, selected_text:) }
  ...
  context 'when there are multiple tokens' do
    ...
    let(:selected_tokens) { [token1, token2, token3] }
    let(:token) { token2 }
    ...
    context 'when there is only value_before' do
      let(:value_before_selected_text) { token1.t[0..2] }
      let(:selected_tokens) { [token1, token2] }
    ...
    context 'when there should be only value_after' do
      ...
      let(:selected_tokens) { [token2, token3] }
  ...
  context 'when there is only one token' do
    let(:token) { create(:token, index: 0) }
    let(:selected_tokens) { [token] }
    let(:value_before_selected_text) { '' }
    let(:selected_text) { token.t }
